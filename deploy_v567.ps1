...
Write-Host "DEPLOY v567 - Orden Predicacion" -ForegroundColor Cyan
$ok = 0; $fail = 0
foreach ($f in $archivos) {
    ...
    try {
        ...
        $ok++
    } catch { Write-Host ("  ERROR: " + $f) -ForegroundColor Red; $fail++ }
}
if ($fail -eq 0) { Write-Host "DEPLOY v567 COMPLETADO!" -ForegroundColor Green }
