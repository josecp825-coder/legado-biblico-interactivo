...
(function () {
    ...
    window._AB_iniciarPlan = function(planKey) {
        ...
        if (!confirm('¿Comenzar "' + PLANES[planKey].nombre + '"?\n\nTu lectura empieza HOY.\nCada capítulo que leas lo marcas de verde.')) return;
        ...
        var datos = obtenerDatos();
        if (datos) _renderDashboard(datos, datos.diaEnCurso);  // ← SIEMPRE donde se quedó
    };
    ...
    window._AB_confirmarCambiarPlan = function() {
        if (!confirm('¿Cambiar de plan?\n\nSe perderá tu progreso.')) return;
