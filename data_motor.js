...
async function cambiarCapituloGlobal(delta) {
    ...
    if (nuevoCap > 0) abrirLibroPrincipal(currentLibroNombre, nuevoCap);
}
...
function buscarEnCapitulo(query) {
    ...
    blocks.forEach(block => {
        ...
        const texto = block.textContent.toLowerCase();
        if (texto.includes(q)) {
            block.style.display = '';
    ...
    if (q && count === 0) mostrarToast(`🔍 No se encontró "${query}" en este capítulo`);
}
