                ...
                <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(136, 84, 208, 0.3);padding:25px;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,0.5);">
                    <div style="display:grid;gap:15px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:10px;">
                            <label style="display:block;cursor:pointer;background:rgba(0,0,0,0.5);border:2px solid rgba(136, 84, 208, 0.8);border-radius:12px;padding:12px;" onclick="const e=document.getElementById('reg-fecha'); if(e.showPicker) e.showPicker();">
                                <div style="color:#fed330;font-weight:bold;font-size:0.8rem;letter-spacing:1px;text-align:center;margin-bottom:6px;">📅 TOCA AQUÍ PARA ELEGIR LA FECHA</div>
                                <input type="date" id="reg-fecha" value="${hoyStr}" onchange="if(typeof autoSincronizarDia === 'function') autoSincronizarDia()" style="width:100%;padding:8px;background:transparent;border:none;color:#fff;outline:none;font-size:1.1rem;font-weight:900;text-align:center;cursor:pointer;display:block;" required onclick="try{this.showPicker()}catch(e){}">
                            </label>
                            <select id="reg-dia" onchange="actualizarVisibilidadTestimonios()" style="display:none;">
...
<div style="display:flex;gap:6px;margin-top:2px;">
                        ...
                        <div style="background:rgba(116,185,255,0.04);border:1.5px solid rgba(116,185,255,0.2);border-radius:10px;padding:12px;">
                            <label style="color:#74b9ff;font-size:0.75rem;font-weight:bold;">🎛️ Encargado de Sonido</label>
                            <input type="text" id="reg-sonido" placeholder="Nombre del encargado de sonido..." style="width:100%;padding:12px;background:rgba(0,0,0,0.2);border:1px solid rgba(116,185,255,0.3);color:#fff;border-radius:8px;margin-top:5px;outline:none;" class="flecha-nombres" oninput="if(typeof autocompleteNombre==='function')autocompleteNombre(this)" onfocus="if(typeof autocompleteNombre==='function')autocompleteNombre(this, true)" onclick="if(typeof autocompleteNombre==='function')autocompleteNombre(this, true)" autocomplete="off">
                        </div>
                        ...
                        <div style="display:grid;grid-template-columns:1fr;gap:12px;margin-top:25px;">
                            ...
                            <button onclick="typeof accionGenerarPlantilla === 'function' ? accionGenerarPlantilla('semana') : alert('Programando pronto...')" style="padding:14px;background:linear-gradient(135deg,#0984e3,#74b9ff);border:none;color:#fff;font-weight:900;border-radius:12px;cursor:pointer;font-size:0.95rem;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 6px 20px rgba(9,132,227,0.3);border:1px solid rgba(255,255,255,0.2);">
                                2. ✨ PLANTILLA
                            </button>
                            
                            <button onclick="typeof accionGenerarPDF === 'function' ? accionGenerarPDF('semana') : alert('Programando pronto...')" style="padding:14px;background:linear-gradient(135deg,#ff4757,#ff6b81);border:none;color:#fff;font-weight:900;border-radius:12px;cursor:pointer;font-size:0.95rem;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 6px 20px rgba(255,71,87,0.3);border:1px solid rgba(255,255,255,0.2);">
                                3. 📄 PDF
                            </button>
                            ...
                            <button onclick="delegarFormularioRegular()" style="padding:14px;background:linear-gradient(135deg,rgba(37,211,102,0.15),rgba(37,211,102,0.05));border:1px dashed rgba(37,211,102,0.5);color:#2ed573;font-weight:900;border-radius:12px;cursor:pointer;font-size:0.95rem;display:flex;align-items:center;justify-content:center;gap:8px;">
                                4. 📲 DELEGAR CULTO (Enviar en blanco)
                            </button>
                        </div>
                    </div>
                </div>
...
// ==========================================
function precargarUltimoCultoSemanal() {
    if (window.regularEditandoId) return; // Si ya está editando uno, no tocar
...
window.autoSincronizarDia = function() {
    const elFecha = document.getElementById('reg-fecha');
    const elDia = document.getElementById('reg-dia');
    if (!elFecha || !elDia) return;

    const fp = elFecha.value;
    if (!fp) return;

    const p = fp.split('-');
    const f = new Date(p[0], p[1]-1, p[2]);
    const dias = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
    const diaCorrecto = dias[f.getDay()];
    
    elDia.value = diaCorrecto;
    if (typeof actualizarVisibilidadTestimonios === 'function') {
        actualizarVisibilidadTestimonios();
    }
};
...
function guardarCultoRegular() {
    ...
    const data = {
        ...
        sonido: document.getElementById('reg-sonido') ? document.getElementById('reg-sonido').value.trim() : ""
    };

    if (!data.fecha) { 
        if (typeof mostrarToast === 'function') mostrarToast('⚠️ Selecciona una fecha válida'); else alert('⚠️ Selecciona una fecha válida');
        return; 
    }
    ...
    if (regularEditandoId) {
        ...
        if (typeof _syncCultoRegularFirebase === 'function') {
            _syncCultoRegularFirebase(data).catch(e => console.warn('Firebase error:', e));
        }
    }
...
function editarCultoRegular(id) {
    try {
        console.log('[CULTO-DEBUG] editarCultoRegular INICIO - ID:', id);
        let registros = JSON.parse(localStorage.getItem('legado_cultos_regulares') || '[]');
        console.log('[CULTO-DEBUG] Total registros en localStorage:', registros.length);
        ...
        const elFecha = document.getElementById('reg-fecha');
        if (elFecha) elFecha.value = reg.fecha || '';
        
        const elDia = document.getElementById('reg-dia');
        if (elDia) elDia.value = reg.dia || 'miercoles';
        
        if (typeof actualizarVisibilidadTestimonios === 'function') {
            try { actualizarVisibilidadTestimonios(); } catch (e) { console.warn(e); }
        }
...
function ejecutarBorradoRegular(id) {
    ...
    // 🔥 FIREBASE SYNC - GUARD SECURITY
    if (typeof _deleteCultoRegularFirebase === 'function') {
        _deleteCultoRegularFirebase(id).catch(() => {});
    }
...
`;
    if (reg.predicador_quien && reg.predicador_quien !== "-") {
        texto += `${nextNum}️⃣ *Presenta al Predicador:* ${reg.predicador_quien}
`;
    }
    texto += `${nextNum++}️⃣ *PREDICADOR:* ${reg.mensaje || "-"}
`;
    if (reg.mensaje_tema && reg.mensaje_tema !== "-") {
        texto += `      📌 *MENSAJE:* _"${reg.mensaje_tema}"_
`;
    }
...
// ==========================================
function obtenerListasOficiales() {
    let def = {
        ancianos: ['Jose Castillo', 'Jose Luis Candelario', 'Jose Lopez', 'Obed Lopez', 'Luz Lopez', 'Cecilia Wilson', 'Daniel Marquez', 'Haston Tomas', 'Nerlys Maza'],
        diaconos: ['Juan Ernesto', 'Juan Antigua', 'Flavio Candelario', 'Jose Luis Candelario', 'Freddy Vargas', 'Juan Gutierrez', 'Logan Vargas', 'Jhon Chica', 'Engel Candelario', 'Jose Lopez'],
        diaconisas: ['Donil Ortega', 'Fermina Hernandez', 'Luz Lopez', 'Wanda Nuñez', 'Fiordaliza Gutierrez'],
        sonido: ['Jose Luis Candelario', 'Haston Tomas', 'Logan Vargas', 'Engel Candelario', 'Freddy Vargas', 'Jose Castillo']
    };
    let saved = JSON.parse(localStorage.getItem('legado_listas_oficiales') || 'null');
    return saved ? saved : def;
}

function getNombresGlobales(filtro = '') {
    const listas = obtenerListasOficiales();

    let base = [...listas.ancianos, ...listas.diaconos, ...listas.diaconisas, ...listas.sonido];
    let camposHistorial = ['anciano', 'diacono', 'diaconisa', 'alabanza', 'bienvenida', 'oracionIni', 'himnoIni_quien', 'lectura_quien', 'especial_quien', 'predicador_quien', 'mensaje', 'himnoFin_quien', 'oracionFin'];

    if (filtro === 'anciano') {
        base = listas.ancianos;
        camposHistorial = []; // Ignorar historial para no ensuciar la lista oficial
    } else if (filtro === 'diaconisa') {
        base = listas.diaconisas;
        camposHistorial = []; // Ignorar historial
    } else if (filtro === 'diacono') {
        base = listas.diaconos;
        camposHistorial = []; // Ignorar historial
    } else if (filtro === 'sonido') {
        base = listas.sonido;
        camposHistorial = []; // Ignorar historial
    }
    ...
    // Si NO hay filtro estricto, sumamos el historial general
    if (camposHistorial.length > 0) {
        let registrosSemana = JSON.parse(localStorage.getItem('legado_cultos_regulares') || '[]');
        let registrosSabado = JSON.parse(localStorage.getItem('legado_cultos') || '[]');
        let registros = [...registrosSemana, ...registrosSabado];
...
window.autocompleteNombre = function(input, forceShowAll = false) {
    ...
    let filtro = '';
    if (id.includes('anciano')) filtro = 'anciano';
    else if (id.includes('diaconisa')) filtro = 'diaconisa';
    else if (id.includes('diacono')) filtro = 'diacono';
    else if (id.includes('sonido')) filtro = 'sonido';

    let todos = getNombresGlobales(filtro);
...
// ================================================================
// 🔥 FIREBASE SYNC — Cultos Regulares (Miércoles y Viernes)
// ================================================================
async function _syncCultoRegularFirebase(datos) {
    try {
        if (typeof db === 'undefined') return;
        const docId = String(datos.id);
        await db.collection('iglesias').doc('cypress_hills_brooklyn')
            .collection('cultos_regulares').doc(docId).set(datos);
    } catch(e) {}
}

async function _deleteCultoRegularFirebase(id) {
    try {
        if (typeof db === 'undefined') return;
        await db.collection('iglesias').doc('cypress_hills_brooklyn')
            .collection('cultos_regulares').doc(String(id)).delete();
    } catch(e) {}
}

window.sincronizarCultosRegularesDesdeFirebase = async function() {
    try {
        if (typeof db === 'undefined') return;
        const snap = await db.collection('iglesias').doc('cypress_hills_brooklyn')
            .collection('cultos_regulares').get();
        if (snap.empty) return;
        
        const fDatos = snap.docs.map(d => d.data());
        const lDatos = JSON.parse(localStorage.getItem('legado_cultos_regulares') || '[]');
        
        const ids = new Set(lDatos.map(c => String(c.id)));
        const soloFb = fDatos.filter(c => !ids.has(String(c.id)));
        
        if (soloFb.length > 0) {
            const todos = [...lDatos, ...soloFb].sort((a,b) => new Date(b.fecha) - new Date(a.fecha));
            localStorage.setItem('legado_cultos_regulares', JSON.stringify(todos));
            if (typeof renderCultosRegulares === 'function' && document.getElementById('lista-cultos-regulares')) {
                renderCultosRegulares(); // Actualizar interfaz si está abierta
            }
        }
    } catch(e) {}
};
...
if (navigator.onLine) {
    setTimeout(() => {
        if (typeof window.sincronizarCultosRegularesDesdeFirebase === 'function') {
            window.sincronizarCultosRegularesDesdeFirebase().catch(() => {});
        }
    }, 4000); // 4 segundos después de cargar
}
...
window.generarPlantillaSemanaHistorial = function(id) {
    // Leer directamente del localStorage — SIN abrir formulario
    let registros = JSON.parse(localStorage.getItem('legado_cultos_regulares') || '[]');
    let reg = registros.find(r => String(r.id) === String(id));
    if (!reg) {
        if (typeof mostrarToast === 'function') mostrarToast('⚠️ Registro no encontrado');
...
window.generarPDFSemanaHistorial = function(id) {
    // Leer directamente del localStorage — SIN abrir formulario
    let registros = JSON.parse(localStorage.getItem('legado_cultos_regulares') || '[]');
    let reg = registros.find(r => String(r.id) === String(id));
    if (!reg) {
    ...
    if (typeof generarPDFDesdeRegistro === 'function') {
        generarPDFDesdeRegistro(reg);
    } else {
        if (typeof mostrarToast === 'function') mostrarToast('⚠️ Motor de PDF no cargado');
    }
};
