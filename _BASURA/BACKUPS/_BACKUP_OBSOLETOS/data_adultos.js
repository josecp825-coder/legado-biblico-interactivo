    ...
    container.innerHTML = `
        <div style="min-height:100vh;background:linear-gradient(170deg,#1a0a00,#2d1a00,#1a1200);font-family:'Georgia',serif;padding-bottom:50px;">
            ...
            <div style="background:rgba(0,0,0,0.5);backdrop-filter:blur(20px);border-bottom:1px solid rgba(212,175,55,0.2);padding:16px 22px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;">
                <button onclick="volverAlInicioLegado()" style="background:rgba(212,175,55,0.1);border:1px solid rgba(212,175,55,0.3);color:#d4af37;padding:8px 16px;border-radius:6px;cursor:pointer;font-size:0.8rem;font-family:'Georgia',serif;letter-spacing:1px;">← INICIO</button>
    ...
    container.innerHTML = `
        <div style="min-height:100vh;background:linear-gradient(170deg,#0a0818,#130f2e,#0d0a22);font-family:'Georgia',serif;padding-bottom:100px;">
            ...
            <div style="padding:22px 20px;max-width:750px;margin:0 auto;">
                <p style="color:rgba(255,255,255,0.4);font-size:0.8rem;margin:0 0 30px;font-family:'Segoe UI',sans-serif;text-align:center;">Creencias Fundamentales — Stone Oak Spanish SDA Church</p>
                
                ${categorias.map(cat => `
                    <div style="margin-bottom:40px;">
                        <div style="background:rgba(162,155,254,0.05); border-left:4px solid #a29bfe; padding:20px; border-radius:10px; margin-bottom:15px;">
                            <h2 style="color:#a29bfe; font-size:1.1rem; letter-spacing:3px; margin:0 0 8px;">${cat}</h2>
                            <p style="color:rgba(255,255,255,0.6); font-size:0.85rem; line-height:1.5; margin:0; font-family:'Segoe UI',sans-serif;">${intros[cat] || ""}</p>
                        </div>
                        <div style="display:grid;gap:12px;">
                            ${docs.filter(d => d.cat === cat).map(d => `
                                <div onclick="abrirEstudioDoctrina(${d.num})" style="background:rgba(255,255,255,0.03);border:1px solid rgba(162,155,254,0.12);border-radius:12px;padding:20px;transition:0.3s;cursor:pointer;"
                                    onmouseover="this.style.borderColor='rgba(162,155,254,0.3)';this.style.background='rgba(255,255,255,0.05)'"
                                    ...
                                    <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
                                        ...
                                        <h3 style="color:#fff;font-size:0.95rem;margin:0;letter-spacing:0.5px;">${d.titulo}</h3>
                                    </div>
                                    <p style="color:rgba(255,255,255,0.75);font-size:0.88rem;line-height:1.7;margin:0 0 12px;font-family:'Segoe UI',sans-serif;">${d.resumen}</p>
                                    <div style="display:flex;justify-content:space-between;align-items:center;">
                                        <div style="display:flex;align-items:center;gap:8px;opacity:0.6;">
                                            <span style="font-size:0.75rem;">📖</span>
                                            <span style="color:#a29bfe;font-size:0.75rem;font-family:'Segoe UI',sans-serif;font-weight:700;">${d.ref}</span>
                                        </div>
                                        <div style="color:#a29bfe; font-size:0.8rem; font-weight:900;">PROFUNDIZAR →</div>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
